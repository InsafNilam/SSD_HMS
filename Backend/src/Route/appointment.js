...
router.get('/user-appointment/:id', async (req,res)=>{
...
router.get('/report-not/:date', async (req,res) => {
...
router.get('/feed-appointment', async (req,res) => {
...
router.get('/all-appointment/:name', async (req,res) => {
...
router.get('/appointment/:name', async (req,res) => {
...
router.get('/doctor-treatment/:name', async (req,res)=>{
...
router.post('/appointment-time',async (req,res)=>{
...
router.put('/doctor-prescription/:id', async (req,res) => {
    const id = req.params.id.toString();
    if(!isValidObjectId(id)) {
        res.set('Content-Type', 'text/plain');
        return res.status(400).send(`No Record with given id : ${id}`)
...
router.put('/app-feedback/:id', async (req,res)=>{
    const id = req.params.id.toString();
    if(!isValidObjectId(id)) {
        res.set('Content-Type', 'text/plain');
        return res.status(400).send(`No Record with given id : ${id}`);
...
router.get('/user-treatment/:id', async (req,res)=>{
    const id = req.params.id.toString();
    if(!isValidObjectId(id)) {
        res.set('Content-Type', 'text/plain');
        return res.status(400).send(`No Record with given id : ${id}`);
